...
#
# Taken from https://github.com/wiseleyb/library-of-congress-scraper
...
class ScrapeLOC
  def initialize(name, base_url)
    ...
    if @start_page.to_i > 0
      @base_url = "#{@base_url}?sp=#{@start_page}"
...
=end
    puts "[#{@name}] #{@counter/@page_size}"
    parsed.css('.item-description').each do |div|
      span = div.css('.item-description-title')
      a = span.css('a')
      ...
      # abstract = div.css('.item-description-abstract').text
      # puts abstract.strip
      date = div.css('.date').css('span').text
      # puts date
...
=end
      ...
      # puts '*' * 40
      # puts "[#{@name}] #{@counter} page: #{@counter/@page_size}"
      ...
      # puts img_url
      fname = "#{@output_path}/#{to_filename(title, img_url, date)}"
      # puts fname
...
scraper = ScrapeLOC.new(ARGV[0], ARGV[1])
puts "Start Scraping #{ARGV[0]}"
scraper.run
puts "Stop Scraping #{ARGV[0]}"
